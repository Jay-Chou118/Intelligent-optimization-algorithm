...
max_axial_height_R2 = 0.03; % 第二个线圈的最大轴向高度 (m)
...
while temperature > final_temperature && iteration < max_iterations
    ...
    current_height_R2 = 0; % 当前高度（第二个线圈）
     ...
     for i = 1:n1
        % 确保下一层的每个环路下方有上一层的支持，并且总高度不超过最大轴向高度
        ...
        current_height_R1 = current_height_R1 + min_spacing_R1; % 增加高度
        if current_height_R1 >= max_axial_height_R1 % 如果达到最大高度，增加半径并重置高度
            current_layer_R1 = current_layer_R1 + min_spacing_R1;
            current_height_R1 = 0;
        end
        for j = 1:n1
    ...
    for i = 1:n2
        ...
        current_height_R2 = current_height_R2 + min_spacing_R2; % 增加高度
        if current_height_R2 >= max_axial_height_R2 % 如果达到最大高度，增加半径并重置高度
            current_layer_R2 = current_layer_R2 + min_spacing_R2;
            current_height_R2 = 0;
        end
        for j = 1:n2
