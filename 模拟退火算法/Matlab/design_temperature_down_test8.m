...
while temperature > final_temperature && iteration < max_iterations
           ...
           if R1_i >= min_radius_R1 && R1_i <= max_radius_R1 && d_ij >= min_spacing_R1
                if i == j
                   ...
                   % 使用符号计算和高精度计算来计算互感，提高精度
                    ...
                    L1_matrix(i, j) = double(vpa(int(int(M_sym, phi1, 0, 2 * pi), phi2, 0, 2 * pi))); % 互感
                end
            end
        end
    end
        ...
        for j = 1:n2
            ...
            if R2_i >= min_radius_R2 && R2_i <= max_radius_R2 && d_ij >= min_spacing_R2
                if i == j
                    ...
                    d_sym = d_ij;
                    M_sym = mu_0 * R1_sym * R2_sym / sqrt((R1_sym * cos(phi1) - R2_sym * cos(phi2))^2 + (R1_sym * sin(phi1) - R2_sym * sin(phi2))^2 + d_sym^2);
                    L2_matrix(i, j) = double(vpa(int(int(M_sym, phi1, 0, 2 * pi), phi2, 0, 2 * pi))); % 互感
                end
            end
        end
    end
    ...
    for i = 1:n1
     ...
     % 分块求和以避免数值溢出
    L1_total = sum(sum(mat2cell(L1_matrix, repmat(ceil(size(L1_matrix, 1)/10), 1, 10), repmat(ceil(size(L1_matrix, 2)/10), 1, 10)), 'UniformOutput', false));
    L2_total = sum(sum(mat2cell(L2_matrix, repmat(ceil(size(L2_matrix, 1)/10), 1, 10), repmat(ceil(size(L2_matrix, 2)/10), 1, 10)), 'UniformOutput', false));
